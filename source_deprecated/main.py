...
import source_deprecated.Visu as vi
import source.mc as mc
import time
import source.reader
import source.model as md
from source_deprecated import Test_true as ts
...
### =======
md.draw_factor = 0
data = source.reader.read_data('../data/basketball/nba.csv')
N = data.shape[0]
data_training = data[:N/2,:]
data_test = data[N/2:,:]
...
ite_gibbs = 4000
tic = time.time()
source.mc.gibbs_mcmc(p, m, ite_gibbs)
...
agr = 0.8
tic = time.time()
decisions = source.mc.mh_mcmc(p, m, agr, ite_metro)
